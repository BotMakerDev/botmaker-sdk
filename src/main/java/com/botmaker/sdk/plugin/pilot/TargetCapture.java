...
public final class TargetCapture {
    ...
    public TargetCapture(PilotProject project) {
        this(project, () -> 0);
    }

    public TargetCapture(PilotProject project, LongSupplier sessionHostWindow) {
    ...
    public static TargetCapture forProject(PilotProject project) {
        Path resourcesDir = project == null ? null : project.resourcesDir();
        if (resourcesDir == null) {
            return new TargetCapture(project);
        }
        BackgroundLauncher launcher = BackgroundLauncher.forProject(resourcesDir);
        return new TargetCapture(project, launcher::hostWindowId);
    }
