 ...
 */
public final class EmulatorPicker {
    ...
    private EmulatorPicker() {}

    /**
     * The host services for the showing currently on screen, so the dozen static helpers below can theme a
     * child dialog without every one of them taking a parameter it only forwards.
     *
     ...
     * {@link ConnectPhone} writes shared's saved list, and re-scanning is how the row for it appears
    ...
    private static Button connectPhoneRow(VBox rows, Dialog<Selection> dialog) {
        ...
        connect.setOnAction(e -> {
            ConnectPhone.show(host, windowOf(dialog));
            populate(rows, dialog);
        });
        return connect;
    }
    ...
    private static VBox buildRow(EmulatorInstance instance, Dialog<Selection> dialog) {
        ...
        if (instance.platformId() == PlatformId.WAYDROID) {
            ...
            diagnose.setOnAction(e -> WaydroidReport.show(host, windowOf(dialog)));
     ...
     * exact moment {@link WaydroidReport} was written for: the command dispatched fine and the
        ...
        new Thread(() -> {
            ...
            Platform.runLater(() -> {
                ...
                if (!settled && start && instance.platformId() == PlatformId.WAYDROID) {
                    WaydroidReport.show(host, windowOf(dialog));
                }
