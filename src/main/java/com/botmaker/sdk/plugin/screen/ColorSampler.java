 ...
 * </ul>
 *
 * <p>The frame is frozen rather than live — unlike the host's own screen overlay, which is a transparent
 * window onto the moving desktop. Freezing is what allows zooming, and it means the pixel you inspect is the
 * pixel you get, with no chance of the game repainting between the look and the click.
 *
 * <p><b>What it asks of the host is theming, a window to be owned by, and the AWT→FX conversion</b> — all of
 * it already on {@link StudioServices}. The frame itself comes from {@link EditorFrame}, which is this
 * plugin's own: the pixels a bot will look at are the SDK's subject, not the editor's.
 */
public final class ColorSampler {
    ...
    public record Sample(java.awt.Color color, EditorFrame frame, double spread) {
    ...
    /**
     * Opens the sampler over an already-grabbed {@code frame}.
     *
     * <p>Takes a frame rather than fetching one, and that is the whole of the split with {@link EditorFrame}:
     * a caller that already has pixels in hand — a surface the user is looking at — must not re-grab them,
     * because the frame it is showing and the frame it samples have to be the same one.
     */
    public static void openOn(StudioServices services, EditorFrame frame, Consumer<Sample> onPicked) {
    ...
    private static final class Surface {
        ...
        Surface(StudioServices services, EditorFrame frame, Consumer<Sample> onPicked) {
            this.frame = frame;
            ...
            Scene scene = services.theme().scene(root);
