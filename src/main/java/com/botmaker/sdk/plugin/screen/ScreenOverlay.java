 ...
 * Which pixels to grab is {@link ShotSource}'s question, and {@link DesktopSource} is what answers it here —
 * a pick with nothing but a slot in hand is a pick over this machine's screens. A pick over what the bot
 * actually looks at goes through {@link EditorFrame} instead, which is asked for by name.
 *
 * <p><b>It was the host's, behind the contract's {@code Capture}, and that member is deleted.</b> The
 * argument for keeping it there was that only a host can put a surface over its own windows. That is true and
 * it is not the point: a full-screen overlay over a running game, asking the user to point at something in
 * it, is about what a bot sees from end to end, and the editor's part in it was only that the editor was
 * written first. So a plugin draws its own overlay now, over pixels it grabbed itself through
 * {@code botmaker-shared}, and the contract keeps what only a host can answer.
 *
...
public final class ScreenOverlay {
    ...
    /**
     * @param source where frames come from; never {@code null} — {@link DesktopSource} in this module
     */
     ...
     * thumbnail (cropped from {@code desktop}). The row matching the remembered default is preselected and
     * the chosen index is saved back, so the next capture defaults to it. Returns the chosen {@link Screen},
     * or {@code null} if cancelled.
     *
     * <p>The remembering used to be Studio's {@code ProjectPreferences}, a host preference file. This is a
     * plugin now and has no host preferences to write into — nor should it ask for any, since <em>which
     * monitor this user last captured</em> is not something only the host can know. {@link Preferences} is
     * the JDK's own per-user store and needs nobody's permission.
     */
