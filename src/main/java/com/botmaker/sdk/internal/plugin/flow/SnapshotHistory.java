 ...
 * eight mutations (move, link, unlink, switch on/off, set start, add, delete, auto-arrange) and several of
 * them have side effects on each other — deleting a card drops its wires, disabling the start activity moves
 * the start, renaming rewires every edge that named it. Each inverse would have to know all of that, and the
 * day one forgets is the day undo leaves a wire pointing at a card that isn't there. A snapshot cannot
 * forget: {@code before} is simply what the state was, and undo puts it back.
 *
 * <p>Snapshots are cheap because what they hold is small — a few dozen cards, each a name, two doubles and a
 * flag. The card <em>objects</em> are shared, not copied, and that sharing is deliberate: the drafts are
 * bound to the side panel's fields, so an undo has to bring back the very object the panel is editing, not an
 * equal one. It is also why {@code S} must have a value-based {@code equals} — a no-op "mutation" (a drag
 * that put the card back where it started) is dropped by comparing before with after, and nothing else can
 * tell.
 *
 * <p><b>This is a copy of the host's own {@code state.SnapshotHistory}, and a copy is the right answer.</b>
 * The editor keeps one for the code editor's history and for the Parameters window; this plugin needs the
 * same machinery for a window the host can no longer see. Sharing it would mean either a contract member for
 * an undo stack — a capability nothing but this would ask for, and the back door the platform exists to
 * refuse — or a widget kit dependency for something that draws nothing. Forty lines of {@code ArrayDeque} is
...
public final class SnapshotHistory<S> {
    ...
    /**
     * Records a step whose two halves the caller already holds — the form for a state this history cannot
     * read on demand.
     */
