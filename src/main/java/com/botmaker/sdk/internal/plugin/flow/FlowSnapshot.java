 ...
 * exactly where the mutation did. That is what lets {@link SnapshotHistory} drop a no-op step by comparing
 * them.
 *
 * <p>Names, descriptions and outcomes are <em>not</em> here: those are edited as text, and a rename rewrites
 * every wire that mentioned it, which no earlier snapshot could be restored across.
...
public record FlowSnapshot(List<CardState> cards, List<Flow.Edge> edges, String start) {
    ...
    /** This snapshot with {@code draft}'s enable flag set to {@code enabled} — see {@link SnapshotHistory#commit}. */
